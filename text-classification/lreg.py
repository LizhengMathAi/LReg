...
class LagrangeEncoder(torch.nn.Module):
    def __init__(self, n_components: int, dof: int):
        super(LagrangeEncoder, self).__init__()
    ...
    def forward(self, x):
        with torch.no_grad():
            ...
            print(x[..., None].shape, self.bin_edges.shape)
    ...
    def count_intervals(cls, coarse_nodes: torch.Tensor, inputs: torch.Tensor, losses: torch.Tensor, topk=1) -> torch.Tensor:
        ...
        Example:
            ...
            >>> topk = 3
            >>> counts = LagrangeRegressor.count_intervals(coarse_nodes, inputs, losses, topk)
            >>> counts
            tensor([[1, 2, 0, 0],
                    [0, 1, 1, 1]])
        """
    ...
    def select_intervals(cls, counts: torch.Tensor, topk: int) -> torch.Tensor:
        ...
        Example:
            ...
            >>> topk = 2
            >>> indices = LagrangeRegressor.select_intervals(counts, topk)  # [in_channels, topk]
            >>> indices
            tensor([[1, 0],
                    [2, 3]])
        """
    ...
    def refinement(cls, coarse_counts: torch.Tensor, coarse_nodes: torch.Tensor, indices: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Example:
            ...
            >>> fine_counts, fine_nodes = LagrangeRegressor.refinement(coarse_counts, coarse_nodes, indices)
            >>> fine_counts
    ...
    def to_equal_freq(cls, in_values: torch.Tensor, in_bin_edges: torch.Tensor, dof: Optional[int] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        Example:
            ...
            >>> in_values, in_bin_edges = LagrangeRegressor.to_equal_freq(out_values, out_bin_edges)
    ...
    def coarsening(cls, fine_counts: torch.Tensor, fine_nodes: torch.Tensor, dof: int, eps=1) -> torch.Tensor:
        ...
        Example:
            ...
            >>> coarse_nodes = LagrangeRegressor.coarsening(fine_counts, fine_nodes, dof=4, eps=0)
            >>> coarse_nodes
    ...
    def linear_interpolation(cls, x: torch.Tensor, x_values: torch.Tensor, y_values: torch.Tensor) -> torch.Tensor:
        ...
        Example data
            ...
            interpolated_values = LagrangeRegressor.linear_interpolation(x_to_interpolate, x_values, y_values[:, :, None])[:, :, 0]
...
class LagrangeLinear(torch.nn.Module):
    def __init__(self, n_components: int, out_channels: int, dof: int):
        super(LagrangeLinear, self).__init__()
...
class LagrangeRegressor(nn.Module):
    def __init__(self, in_shape: Tuple[int], n_components:int, out_shape: Tuple[int], dof: int, channel_dim: int = 2):
        super(LagrangeRegressor, self).__init__()
        self.in_shape = in_shape
        self.out_shape = out_shape
        self.channel_dim = channel_dim

        self.sn = SigmoidNormalization(in_channels=in_shape[self.channel_dim], n_components=n_components)
        self.encoder = LagrangeEncoder(n_components=n_components, dof=dof)
        self.head = LagrangeLinear(n_components=n_components, out_channels=out_shape[self.channel_dim], dof=dof)
...
def layer_wrapper(layer: nn.Linear, in_shape: Tuple[int], n_components:int, out_shape: Tuple[int], dof: int, channel_dim=2):
    device = next(layer.parameters()).device
    layer.residual = LagrangeRegressor(in_shape, n_components, out_shape, dof, channel_dim).to(device)
    
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        return self.__class__.forward(self, input + self.residual(input))
...
def add_remainder(model: nn.Module, forward: Callable, in_shape: Tuple[int], n_components: int, out_shape: Tuple[int], dof: int, channel_dim: int=1) -> nn.Module:
    ...
    remainder = LagrangeRegressor(in_shape, n_components, out_shape, dof, channel_dim).to(device)
...
if __name__ == "__main__":
    batch_size, n_components, out_channels, dof = 100, 3, 1, 16
    x = torch.einsum("i,jk->ijk", torch.linspace(-2, 2, batch_size), torch.ones(4, n_components))

    encoder = LagrangeEncoder(n_components=n_components, dof=dof)
    y = encoder(x)

    head = LagrangeLinear(n_components=n_components, out_channels=1, dof=dof)
    head.coeff.data = torch.einsum("d,no->ndo", torch.sin(8 * torch.linspace(-1, 1, dof)), torch.ones(n_components, 1))
    z = head(y)
    ...
    for k in range(dof):
        ax.plot(x[:, 0, 0], y[:, 0, 0, k])
    ax.plot(x[:, 0, 0], z[:, 0, 0].detach().numpy())
    plt.show()
